...
def search_intent(labels):
    url = 'https://search-photos-fdkohr7xq5ey3n3h4su3ktqm5m.us-east-1.es.amazonaws.com/photo/_search?q='
    resp = []
    for label in labels:
        if (label is not None) and label != '':
            url2 = url+label
            resp.append(requests.get(url2,auth=('Aakash', 'Elastic@369')).json())
    print (resp)
